...
```{r}
synthetic_gen <- function(background, background_sd, peak, peak_sd, ar_uncertainty, degla1_begin, degla1_end, degla2_begin, degla2_end, small){
  ...
  if(small==1){
      for (i in 1:nrow(syn)){
        ...
        } else if(isTRUE(i>=(t4-5) && i<=(t4-4)) || isTRUE(i>=(t2-5) && i<=(t2-4))){
            syn$events[i] <- as.integer(rnorm(1, mean=pk, sd=pksd))
            if(syn$events[i] < 0){
              syn$events[i] <- 0
            }
...
```{r}
mc_syn <- function(type, p, run, background, background_sd, peak, peak_sd, ar_uncertainty, degla1_begin, degla1_end, degla2_begin, degla2_end, small){
  ...
  if(type %in% "linear"){
    ...
    for(mc in 1:run){
      ...
      df <- synthetic_gen(bg, bgsd, pk, pksd, ar_uncertainty, t1, t2, t3, t4, small)
...
mc_syn(type, prob, runtime, background, background_sd, peak, peak_sd, ar_uncertainty, degla1_begin, degla1_end, degla2_begin, degla2_end, small)
...
## examples
linearDF <- mc_syn("linear", 0.25, 100, 5, 2, 30, 2.5, 0.1, 127, 123, 18, 14, 0)
invereseDF <- mc_syn("inverse", 0.1, 100, 5, 2, 25, 2.5, 0.05, 128, 124, 18, 14, 1)
logDF <- mc_syn("log", 0.25, 100, 3, 1.5, 30, 2, 0.08, 127, 123, 18, 14, 1)
errorExample <- mc_syn("shan", 0.25, 100, 10, 2, 30, 3, 0.03, 127, 123, 18, 12, 0)
...
```{r}
result <- mc_syn("log", 0.2, 500, 5, 2, 30, 2.5, 0.1, 127, 123, 18, 14, 1)
